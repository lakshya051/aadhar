...
def extract_info(text):
    # Implement the logic to extract information based on the content
    # This is a simplified version, you may need to adapt it based on your specific requirements
    data = {}
    ...
    for key, value in data.items():
        if value:
            data[key] = value.group().strip() if hasattr(value, 'group') else value.strip()
        else:
            data[key] = "Not found"
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if st.button("Process Image"):
            ...
            img = cv2.resize(img_array, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
